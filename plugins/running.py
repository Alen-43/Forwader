...
async def run(bot, message):
    if str(message.from_user.id) not in Config.OWNER_ID:
        return
    buttons = [[
        InlineKeyboardButton('🚫sᴛᴏᴘ🚫', callback_data='stop_btn')
    ]]
    reply_markup = InlineKeyboardMarkup(buttons)
    m = await bot.send_message(
        text="ʜᴇʏ! ʙʀᴏ, ғɪʟᴇ ғᴏʀᴡᴀʀᴅɪɴɢ sᴛᴀʀᴛᴇᴅ😉",
   ...
   # await m.delete()
    buttons = [[
        InlineKeyboardButton('📜 sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ', url='https://t.me/space4cinemas')
    ]] 
    reply_markup = InlineKeyboardMarkup(buttons)
    await m.edit(
        text=f"<u><i>sᴜᴄᴄᴇssғᴜʟʟʏ ғᴏʀᴡᴀʀᴅᴇᴅ🥳</i></u>\n\n<b>Total Forwarded Files:-</b> <code>{files_count}</code> <b>Files</b>\n<b>Thanks For Using Me❤️</b>",
        reply_markup=reply_markup
    )
